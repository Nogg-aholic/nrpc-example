...
export const sdkPluginApiRpcDefinition = {
	"text": {
...
export const sdkPluginApiCodecShapeEntries: ReadonlyArray<readonly [string, { args: GeneratedCodecShape; result: GeneratedCodecShape }]> = [
...
export const sdkPluginApiCodecEntries = sdkPluginApiCodecShapeEntries.map(([methodName, shape]) => [methodName, createGeneratedRpcMethodCodec(shape.args, shape.result)] as const);

export const sdkPluginApiCodecRegistry = createRpcCodecRegistry(sdkPluginApiCodecEntries as ReadonlyArray<readonly [string, RpcMethodCodec<any[], any>]>);
...
const attachGeneratedRpcMethodMetadata = <TMethod extends RpcMethodRef<any[], any>>(target: TMethod, methodName: string): TMethod => {
	...
	withRpcMethodCodec(target, sdkPluginApiCodecRegistry(methodName) as RpcMethodCodecFromRef<TMethod>);
	return target;
};

const sdkPluginApiMethodMetadataEntries = [
	[sdkPluginApiRpcDefinition["text"]["analyzeTitle"], "text.analyzeTitle"] as const,
	[sdkPluginApiRpcDefinition["text"]["formatName"], "text.formatName"] as const,
	[sdkPluginApiRpcDefinition["text"]["formatName_string"], "text.formatName_string"] as const,
] as const;

for (const [target, methodName] of sdkPluginApiMethodMetadataEntries) {
	attachGeneratedRpcMethodMetadata(target, methodName);
}

export const sdkPluginApiHttpRouteManifest: HttpRouteManifest = {
  "id": "sdkPlugin",
  "rootPath": [
    "api",
    "plugins",
...
export function installSdkPluginApiNamespace<TTarget extends Record<string, unknown>, TSurface>(
	target: TTarget,
	surface: TSurface,
): TTarget {
	return mountRpcNamespace(target, sdkPluginApiHttpRouteManifest, surface);
}
